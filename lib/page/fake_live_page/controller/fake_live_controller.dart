...
class FakeLiveController extends GetxController {
  ...
  void updateGiftUsers() {
    ...
    fakeuser.asMap().forEach(
      (index, element) {
        randomUsers.add(PkGiftTopUserModel(
            userId: UserId(isProfilePicBanned: false, name: element.user!.name, image: element.user!.image, id: element.user!.id),
            id: "live_$index",
            totalCoins: random.nextInt(100000)));
      },
    );
    ...
    fakeuser.asMap().forEach(
      (index, element) {
        randomUsers2.add(PkGiftTopUserModel(
            userId: UserId(isProfilePicBanned: false, name: element.user!.name, image: element.user!.image, id: element.user!.id),
            id: "live_$index",
            totalCoins: random.nextInt(100000)));
      },
...
// >>>>> >>>>>> >>>> >>>>> >>>>> SOCKET EMIT METHOD <<<<< <<<<< <<<<< <<<<< <<<<<
  ...
  void onLiveViewer() async {
    ...
    fakeuser.asMap().forEach(
      (index, element) {
        liveViewers.add(LiveViewerUserModel(
